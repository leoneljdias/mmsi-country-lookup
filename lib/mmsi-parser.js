...
function getMIDFromMMSI(mmsi) {
  ...
  if (mmsiStr.startsWith("00")) {
    return mmsiStr.substring(2, 5); // 00MIDXXXX → extract MID
  } else if (mmsiStr.startsWith("111")) {
    return mmsiStr.substring(3, 6); // 111MIDXXX → extract MID
  } else if (mmsiStr.startsWith("99")) {
    return mmsiStr.substring(2, 5); // 99MIDXXXX → extract MID
  }

  // Default: first 3 digits for ships (MIDXXXXXX)
